...
> Where NLM's R@5 baselines come from, what they measure, and how to reproduce or extend them.
...
For NLM, the unit is a session, not a chunk or fact. R@5 = 0.972 means NLM surfaced the right session in the top 5 results on 97.2% of evaluator queries (LongMemEval-S public benchmark, n=500, hybrid, body-only). On the 14-month personal corpus the figure is R@5 = 0.900 (90.0%, 36/40 curated decision queries, keyword mode).
...
### 2. Personal 14-month corpus (R@5 90.0%)

The personal-corpus R@5 figure is **90.0% (36/40)**, measured on a 14-month real-usage corpus — the author's actual session history across Claude Code, Codex, Hermes, and others, with a curated evaluator query set drawn from real "what did we decide about X" / "how did Y end up" / "is Z still open" questions. Source: [reports/recall-experiments/2026-06-11-ranking-phase2.md](../reports/recall-experiments/2026-06-11-ranking-phase2.md) (Arm 1, keyword mode with metadata tiebreaker).

**Classifier used to produce the labels:** `deepseek-v4-flash` (DeepSeek cloud). The label, entity, decision, and open-question fields that keyword retrieval scores against were all generated by DeepSeek V4. This matters because — as noted in "Why keyword scores so high" below — the classifier is the moat for keyword performance, and a different classifier will produce different labels and entity sets. Substituting `qwen3:4b-instruct-2507-q4_K_M` (see the [2026-06-02 head-to-head bench](../reports/classifier-comparison/2026-06-02-deepseek-v4-vs-qwen3.md)) is a statistical tie with DeepSeek V4 on schema validity, entity counts, and decision counts, with better open-question coverage; the end-to-end R@5 delta on this 14-month corpus has not yet been published. The shipped local default is now `qwen3.5:4b`; its measured fixture scores are in the baselines table of [docs/classifier-tiers.md](classifier-tiers.md). Treat 90.0% as a ceiling that depends on a frontier-tier classifier, not as the floor for the default install.
...
- **Without classifier output**: a fresh install with no classified sessions has nothing for keyword to score against. Body-only fallback works but is closer to grep than to the 90.0% personal-corpus figure.
- **With a smaller classifier**: a sub-3B local model (e.g. `phi4-mini`, `llama3.2:3b`) produces sparser entity tags and lower-quality decision/open-question extraction than DeepSeek V4 did on the 14-month corpus. `qwen3:4b-instruct-2507-q4_K_M` closes most of this gap on classifier-output quality per the [2026-06-02 bench](../reports/classifier-comparison/2026-06-02-deepseek-v4-vs-qwen3.md), but the end-to-end R@5 delta on the personal corpus is still on the to-publish list. The shipped local default is `qwen3.5:4b` (measured in [docs/classifier-tiers.md](classifier-tiers.md)).
...
Pass through additional flags after a trailing `--` (e.g. `npm run bench:classifier -- ollama:qwen3.5:4b --limit 100 --modes keyword,hybrid`).
