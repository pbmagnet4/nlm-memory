...
- **With a smaller classifier**: a sub-3B local model (e.g. `phi4-mini`, `llama3.2:3b`) produces sparser entity tags and lower-quality decision/open-question extraction than DeepSeek V4 did on the 14-month corpus. The current recommended local default (`qwen3:4b-instruct-2507-q4_K_M`) closes most of this gap on classifier-output quality per the [2026-06-02 bench](../reports/classifier-comparison/2026-06-02-deepseek-v4-vs-qwen3.md), but the end-to-end R@5 delta on the personal corpus is still on the to-publish list.
