...
**Upsert preservation semantic (ratified amendment, Stage A review 2026-07-06).** Session upserts write `scope = COALESCE(new, existing)`: a re-ingest that derives no scope (including any flag-off re-ingest or reprocess) preserves a previously stamped scope instead of nulling it. This is a deliberate deviation from a literal "flag-off stamps NULL on every write" reading: preservation is the chosen semantic because a stamped scope is derived evidence that a NULL-deriving rewrite should not silently destroy. A flag-on re-ingest that derives a scope still updates the stored value. Consequence for tooling: scope corrections and removals go through direct UPDATE tooling (the backfill command and future operator commands), never through re-ingest; the backfill command's "only fill NULL" rule composes with this (it never overwrites, upsert never downgrades).
