...
| 2 | Consumption: agents actually use the memory | Ambient usefulness 18.2% all-band / 7.9% thin (ceiling confirmed by the +2.5pt context-recall gate); pull baseline 2.7/day pre-pivot; post-pivot window UNREAD | Majority of active sessions issue >= 1 pull; pull-path usefulness >= 50% on the locked judge; recall contract iterated on that data |
