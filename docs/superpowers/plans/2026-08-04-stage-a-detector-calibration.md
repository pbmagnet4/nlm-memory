...
- **Survives a mid-run model eviction.** `chatOnce` retries once with no backoff, which is the policy the published replay eval used against a warm endpoint. A 303-call run is different: LM Studio swaps resident models, and a cold load measured 42 seconds on 2026-08-04, so two fast attempts can both land inside one swap. Do not change `chatOnce` (that would alter the replay eval's semantics). Instead the runner wraps each row in its own try/catch, records the failure, and **continues** rather than aborting; after the main pass it retries only the failed rows, once, with a 30-second wait first. The disk cache means neither pass re-pays for rows already judged.
