...
export async function executeSupersede(deps, opts) {
    ...
    const existingSupersededBy = await readExistingSupersededBy(deps.store, predecessor.session.id);
    if (existingSupersededBy === successor.session.id) {
        deps.io.info(`Link already existed (no-op): ${predecessor.session.id} ⇢ ${successor.session.id}`);
        return {
            kind: "noop",
    ...
    if (existingSupersededBy !== null && !opts.yes) {
        ...
        const overwriteOk = await deps.io.confirmOverwrite(predecessor.session, existingCandidate, successor.session);
        if (!overwriteOk) {
    ...
    if (!opts.yes) {
        ...
        if (!ok)
            return { kind: "cancelled", reason: "user-declined-confirm" };
    }
    try {
...
async function resolveSession(deps, argValue, role) {
    if (argValue) {
    ...
    if (query === null)
        return { kind: "cancelled", reason: "user-cancelled-query" };
    if (query.length === 0) {
    ...
    if (picked === null)
        return { kind: "cancelled", reason: "user-cancelled-pick" };
...
async function readExistingSupersededBy(store, predecessorId) {
    const session = await store.getById(predecessorId);
    return session?.supersededBy ?? null;
}
async function resolveCandidateById(store, id) {
...
export function defaultIO() {
    return {
        ...
        async confirmOverwrite(pred, existing, replacement) {
            log.warn(`${formatHandle(pred)} is already marked superseded by ${formatHandle(existing)}.`);
            const r = await confirm({
...
function formatHandle(c) {
    const label = c.label.length > 56 ? `${c.label.slice(0, 55)}…` : c.label;
    const date = c.startedAt ? ` (${c.startedAt.slice(0, 10)})` : "";
    return `“${label}”${date}`;
}
export async function runSupersedeCommand(args, factory = buildDefaultDeps) {
    const deps = factory();
    try {
...
function resolveDefaultPaths() {
    ...
    const migrationsDir = process.env["NLM_MIGRATIONS_DIR"] ??
...
function closeStoreIfPossible(store) {
    const maybe = store;
    if (typeof maybe.close === "function")
        maybe.close();
}
//# sourceMappingURL=supersede.js.map