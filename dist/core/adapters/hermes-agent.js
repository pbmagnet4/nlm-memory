...
export function defaultDbPath() {
    if (process.env["NLM_HERMES_AGENT_DB_PATH"])
        return process.env["NLM_HERMES_AGENT_DB_PATH"];
...
function extractTurns(messages) {
    ...
    for (const msg of messages) {
        ...
        if (msg.role === "tool") {
            ...
            if (raw)
                segments.push(`[tool_result: ${name}: ${preview}${ellipsis}]`);
            else
                segments.push(`[tool_result: ${name}]`);
        }
        else {
            if (msg.content)
            ...
            if (msg.tool_calls) {
                let calls;
                try {
                    calls = JSON.parse(msg.tool_calls);
                }
                catch {
                    calls = [];
                }
...
export class HermesAgentAdapter {
    name = "hermes-agent";
    runtimeVersion = "hermes-agent/1.0";
    transcriptKind = "hermes-agent-sqlite";
    dbPath;
    constructor(opts = {}) {
    ...
    async discover(options) {
        ...
        try {
            ...
            if (options?.since) {
                ...
                rows = db
                    .prepare(`SELECT id FROM sessions WHERE started_at >= ? ORDER BY started_at ASC`)
                    .all(sinceTs);
            }
            else {
                rows = db
    ...
    async parseSession(sessionId) {
        ...
        try {
            ...
            if (!session)
                return null;
            const messages = db
                .prepare(`SELECT id, role, content, tool_calls, tool_name, timestamp
           FROM messages WHERE session_id = ? ORDER BY timestamp ASC`)
            ...
            const label = session.title && session.title.trim()
