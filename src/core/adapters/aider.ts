...
interface ParsedSession {
  ...
  readonly turns: ReadonlyArray<ParsedTurn>;
}

function rawTimestampToId(raw: string): string {
...
function extractTurns(body: string): ParsedTurn[] {
  ...
  for (const chunk of chunks) {
    ...
    if (rest) {
      ...
      if (processed) turns.push({ role: "assistant", text: processed });
    }
  }
  return turns;
}

function parseHistoryFile(filePath: string): ParsedSession[] {
  let content: string;
  try {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (SESSION_HEADER_RE.test(lines[i] ?? "")) sessionStarts.push(i);
  }
  if (sessionStarts.length === 0) return [];

  const sessions: ParsedSession[] = [];
  for (let si = 0; si < sessionStarts.length; si++) {
    const startLine = sessionStarts[si] as number;
    const endLine =
      si + 1 < sessionStarts.length ? (sessionStarts[si + 1] as number) : lines.length;

    const headerLine = lines[startLine] ?? "";
    const match = SESSION_HEADER_RE.exec(headerLine);
    if (!match) continue;

    const rawTimestamp = match[1] as string;
...
export class AiderAdapter implements TranscriptAdapter {
  ...
  async parseSession(id: string): Promise<SessionChunk | null> {
    if (!existsSync(this.historyFile)) return null;
    const sessions = parseHistoryFile(this.historyFile);
    const idx = sessions.findIndex((s) => s.id === id);
    if (idx === -1) return null;

    const session = sessions[idx] as ParsedSession;
    if (session.turns.length === 0) return null;

    const nextSession = sessions[idx + 1] as ParsedSession | undefined;
