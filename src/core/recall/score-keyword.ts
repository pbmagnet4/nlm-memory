...
const EMPTY_SCORE: KeywordScore = { score: 0, matchedIn: [] };

export function scoreKeyword(
  session: Pick<Session, "label" | "summary" | "decisions" | "open">,
  queryTokens: ReadonlySet<string>,
): KeywordScore {
  if (queryTokens.size === 0) return EMPTY_SCORE;
  ...
  for (const d of session.decisions) {
    for (const t of tokenSet(d)) decisionTokens.add(t);
  }
  ...
  for (const o of session.open) {
    for (const t of tokenSet(o)) openTokens.add(t);
  }
