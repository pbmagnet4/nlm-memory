...
At the start of each conversation, NLM's hooks inject a block listing possibly-relevant sessions with their ids and labels. Read this block first. When a session looks relevant, call `get_session` to fetch its full transcript before drawing on it.
