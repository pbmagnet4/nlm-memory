...
`nlm-memory` is a local-first memory layer for AI coding agents. It indexes every session from Claude Code, Codex CLI, OpenCode, Cursor, Windsurf, Hermes WebUI, Hermes Agent, Aider, and pi across nine runtimes into a single searchable store on your machine. Three properties no other memory layer ships together:
...
3. **Strong R@5 baselines.** Two measurements, both committed: **97.2% R@5** on the public LongMemEval-S benchmark (hybrid mode, n=500, body-only, reproducible by anyone) and **90.0% R@5** on a real 14-month personal corpus (n=40 curated decision queries, keyword mode, 36/40 retrieved). No fine-tuning. The corpus labels were generated by DeepSeek V4; expect a lower number with a smaller local classifier. See [docs/benchmarks.md](docs/benchmarks.md) and [docs/methodology-recall-baseline.md](docs/methodology-recall-baseline.md).

**Install-and-recovery validation.** A context-free agent ("the stranger") with no knowledge of NLM internals installed from HEAD and answered "Why pgvector over Qdrant, and was anything reconsidered?" correctly in under 6 minutes, recovering the full decision arc including a superseded session. See [reports/stranger-sim/2026-06-10-recovery-simulation.md](reports/stranger-sim/2026-06-10-recovery-simulation.md).
...
|---|---|---|---|
| GET | `/api/health` | Host-only | Liveness probe: version, warmup + embedding-lane state, corpus stats, `update: {current, latest, behind}` from the daily npm check, and `job: {name, state, processed, total, startedAt, lastAdvanceAt, restarts, restartsTotal} \| null` — the daemon-supervised `reprocess` run (null when idle: never started, or the daemon just restarted, since the supervisor holds no state across restarts). `restarts` is the restarts-without-progress counter (resets on any advance); `restartsTotal` is the run's lifetime respawn count, never reset — it's the one to watch for OOM churn on a run that keeps making progress between crashes. |
...
| `NLM_CLASSIFIER_MODEL` | `qwen3.5:4b` | Model id for the chosen provider. `qwen3.5:4b` (Q4_K_M, 4.7B) is the recommended local default. For measured tier expectations, see [docs/classifier-tiers.md](docs/classifier-tiers.md). |
...
| **R@5 baseline** | 97.2% public benchmark; 90.0% real 14mo corpus ([docs/benchmarks.md](docs/benchmarks.md)) | published varies | published varies | n/a |
...
- [docs/benchmarks.md](docs/benchmarks.md) - all three measurement dimensions: retrieval quality (LongMemEval-S + personal corpus), classifier quality, and consumption quality
- [docs/methodology-recall-baseline.md](docs/methodology-recall-baseline.md) — how both R@5 baselines are measured and how to run LongMemEval-S on your own machine
