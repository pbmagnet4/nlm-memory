...
describe("WorkstreamStore.setLabel / setStatus", () => {
  it("renames a workstream and bumps updated_at", async () => {
    const ws = await storage.workstreams.create("team_local", { id: "ws_1", label: "Old Name", scope: null });
    await storage.workstreams.setLabel("team_local", "ws_1", "New Name");
    const after = await storage.workstreams.getById("team_local", "ws_1");
...
describe("WorkstreamStore.merge", () => {
  it("points from->into, marks merged, and unions entities", async () => {
    ...
    await storage.workstreams.upsertEntities("team_local", "ws_from", ["alpha", "shared"]);
    await storage.workstreams.upsertEntities("team_local", "ws_into", ["shared", "beta"]);
    ...
    const ents = await storage.workstreams.entitiesFor("team_local", ["ws_into", "ws_from"]);
