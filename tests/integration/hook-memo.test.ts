...
describe("hook memo", () => {
  ...
  it("returns an empty set for an unknown conversation", () => {
    expect(loadSurfaced(DEFAULT_TEAM_ID, "conv-1").size).toBe(0);
  });

  it("records and reloads surfaced ids", () => {
    recordSurfaced(DEFAULT_TEAM_ID, "conv-1", ["sess_a", "sess_b"]);
    const got = loadSurfaced(DEFAULT_TEAM_ID, "conv-1");
  ...
  it("accumulates across multiple records and dedups", () => {
    ...
    recordSurfaced(DEFAULT_TEAM_ID, "conv-1", ["sess_a", "sess_c"]);
    expect([...loadSurfaced(DEFAULT_TEAM_ID, "conv-1")].sort()).toEqual(["sess_a", "sess_c"]);
  });

  it("isolates conversations from each other", () => {
    recordSurfaced(DEFAULT_TEAM_ID, "conv-1", ["sess_a"]);
    recordSurfaced(DEFAULT_TEAM_ID, "conv-2", ["sess_z"]);
    expect([...loadSurfaced(DEFAULT_TEAM_ID, "conv-1")]).toEqual(["sess_a"]);
    expect([...loadSurfaced(DEFAULT_TEAM_ID, "conv-2")]).toEqual(["sess_z"]);
  });
  ...
  it("clearSurfaced deletes the memo file and returns true", () => {
    recordSurfaced(DEFAULT_TEAM_ID, "conv-1", ["sess_a"]);
    expect(loadSurfaced(DEFAULT_TEAM_ID, "conv-1").size).toBe(1);
    expect(clearSurfaced(DEFAULT_TEAM_ID, "conv-1")).toBe(true);
    expect(loadSurfaced(DEFAULT_TEAM_ID, "conv-1").size).toBe(0);
  });

  it("clearSurfaced returns false when no memo file exists", () => {
    expect(clearSurfaced(DEFAULT_TEAM_ID, "never-existed")).toBe(false);
  });

  it("clearSurfaced isolates conversations — only deletes the targeted one", () => {
    recordSurfaced(DEFAULT_TEAM_ID, "conv-1", ["sess_a"]);
    recordSurfaced(DEFAULT_TEAM_ID, "conv-2", ["sess_b"]);
    clearSurfaced(DEFAULT_TEAM_ID, "conv-1");
    expect(loadSurfaced(DEFAULT_TEAM_ID, "conv-1").size).toBe(0);
    expect([...loadSurfaced(DEFAULT_TEAM_ID, "conv-2")]).toEqual(["sess_b"]);
  });
  ...
  it("resolveConversationForSession finds the conversation that surfaced a session", () => {
    recordSurfaced(DEFAULT_TEAM_ID, "conv-1", ["sess_a", "sess_b"]);
    recordSurfaced(DEFAULT_TEAM_ID, "conv-2", ["sess_z"]);
    expect(resolveConversationForSession(DEFAULT_TEAM_ID, "sess_a")).toBe("conv-1");
    expect(resolveConversationForSession(DEFAULT_TEAM_ID, "sess_z")).toBe("conv-2");
  });

  it("resolveConversationForSession returns null when no conversation surfaced it", () => {
    recordSurfaced(DEFAULT_TEAM_ID, "conv-1", ["sess_a"]);
    expect(resolveConversationForSession(DEFAULT_TEAM_ID, "never_surfaced")).toBeNull();
  });

  it("resolveConversationForSession picks the MOST RECENT surfacing conversation", () => {
    recordSurfaced(DEFAULT_TEAM_ID, "conv-old", ["sess_shared"]);
    recordSurfaced(DEFAULT_TEAM_ID, "conv-new", ["sess_shared"]);
    // Force conv-old to be older so the tie is deterministic.
    const past = new Date(Date.now() - 60_000);
    utimesSync(join(tmp, "conv-old.json"), past, past);
    expect(resolveConversationForSession(DEFAULT_TEAM_ID, "sess_shared")).toBe("conv-new");
  });
});
