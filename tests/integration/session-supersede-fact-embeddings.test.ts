...
describe("session markSuperseded cascade -- embedding cleanup (sqlite)", () => {
  ...
  it("session markSuperseded cascade deletes embeddings of newly superseded facts", async () => {
    ...
    await sessions.insertSession( "team_local",makeRecord({ id: "sess_a" }), null, null, {
      factStore,
      facts: [fA],
    });
    await factStore.upsertEmbedding("team_local", "f_a", new Float32Array(768).fill(0.1));
    ...
    await sessions.insertSession("team_local", makeRecord({ id: "sess_b" }));
    const fB = makeFact({
      id: "f_b",
      subject: "svc",
      predicate: "framework",
  ...
  it("intra-batch (subject,predicate) duplicate: loser gets no embedding, winner gets one, I7 clean", async () => {
    ...
    await sessions.insertSession( "team_local",makeRecord({ id: "sess_dup" }), embedder, null, {
      factStore,
