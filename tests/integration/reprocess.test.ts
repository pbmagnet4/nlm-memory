...
describe("reprocess", () => {
  ...
  describe("selectReprocessCandidates", () => {
    it("selects sessions with NULL classifier_model", async () => {
      await store.insertSession("team_local", baseRecord({ id: "sess_null", startedAt: "2026-01-01T10:00:00Z" }));
  ...
  describe("reprocess full replacement", () => {
    it("replaces label, summary, entities, decisions, provenance and refreshes embeddings", async () => {
      ...
      const sess = await store.getById("team_local", "sess_replace");
    ...
    it("leaves workstream binding (workstream_id, binding_source, binding_confidence) untouched", async () => {
      await store.insertSession("team_local", baseRecord({ id: "sess_ws", startedAt: "2026-01-01T10:00:00Z" }));
    ...
    it("counts belowFloorOverwrites and updates session row but preserves prior facts", async () => {
      ...
      const sess = await store.getById("team_local", "sess_low_conf");
      expect(sess?.label).toBe("Low confidence");
      expect(sess?.classifierConfidence).toBeCloseTo(0.2);

      const facts = await factStore.listBySession("team_local", "sess_low_conf");
  ...
  describe("dry-run", () => {
    it("returns cohort report without writing anything", async () => {
      ...
      const labelAfter = (await store.getById("team_local", "sess_dry_1"))?.label;
  ...
  describe("resume", () => {
    it("skips sessions whose ids are in the state file", async () => {
      ...
      const doneSess = await store.getById("team_local", "sess_done");
      expect(doneSess?.label).toBe("Old label");

      const todoSess = await store.getById("team_local", "sess_todo");
    ...
    it("invalidates done-set when lane changes", async () => {
      ...
      const sess = await store.getById("team_local", "sess_stale");
  ...
  describe("report fields", () => {
    ...
    it("separates skippedAlreadyDone (state-file) from limitSkipped (--limit cutoff)", async () => {
      await store.insertSession("team_local", baseRecord({ id: "sess_r1", startedAt: "2026-01-01T10:00:00Z" }));
      await store.insertSession("team_local", baseRecord({ id: "sess_r2", startedAt: "2026-01-02T10:00:00Z" }));
      await store.insertSession("team_local", baseRecord({ id: "sess_r3", startedAt: "2026-01-03T10:00:00Z" }));
