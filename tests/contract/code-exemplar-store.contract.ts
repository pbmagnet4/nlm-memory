...
export function runCodeExemplarStoreContract(h: CodeExemplarStoreContractHarness): void {
  describe(`CodeExemplarStore contract: ${h.name}`, () => {
    ...
    it("inserts and retrieves an exemplar by id", async () => {
      ...
      const { id, skipped } = await storage.exemplars.insert("team_local", inp);
      expect(skipped).toBe(false);
      const fetched = await storage.exemplars.getById("team_local", id);
    ...
    it("insert is idempotent on duplicate (scope, repo, code_hash, outcome)", async () => {
      const inp = makeExemplarInput();
      const first = await storage.exemplars.insert("team_local", inp);
      const second = await storage.exemplars.insert("team_local", inp);
    ...
    it("different outcomes for the same code produce distinct rows", async () => {
      ...
      const r1 = await storage.exemplars.insert("team_local", passInp);
      const r2 = await storage.exemplars.insert("team_local", failInp);
    ...
    it("insertMany skips duplicates and returns inserted count", async () => {
      const inp = makeExemplarInput();
      await storage.exemplars.insert("team_local", inp);
      const count = await storage.exemplars.insertMany("team_local", [
        inp,
    ...
    it("pruneReverted deletes survived=0 rows", async () => {
      ...
      const { id: rId } = await storage.exemplars.insert("team_local", reverted);
      await storage.exemplars.insert("team_local", good);
      const deleted = await storage.exemplars.pruneReverted("team_local", "install-test");
      expect(deleted).toBe(1);
      expect(await storage.exemplars.getById("team_local", rId)).toBeNull();
    });
    ...
    it("pruneOlderThan deletes rows with ts < cutoff", async () => {
      ...
      const { id: oldId } = await storage.exemplars.insert("team_local", old);
      const { id: newId } = await storage.exemplars.insert("team_local", recent);
      const deleted = await storage.exemplars.pruneOlderThan("team_local", "2026-06-01T00:00:00.000Z");
      expect(deleted).toBe(1);
      expect(await storage.exemplars.getById("team_local", oldId)).toBeNull();
      expect(await storage.exemplars.getById("team_local", newId)).not.toBeNull();
    });

    it("a freshly inserted exemplar is active, llm-sourced", async () => {
      const { id } = await storage.exemplars.insert("team_local", makeExemplarInput());
      const fetched = await storage.exemplars.getById("team_local", id);
      expect(fetched).not.toBeNull();
    ...
    it("setVerdict retire sets retired_at + label_source", async () => {
      const { id } = await storage.exemplars.insert("team_local", makeExemplarInput());
      const res = await storage.exemplars.setVerdict("team_local", id, { retired: true }, "human");
    ...
    it("setVerdict can relabel outcome and un-retire", async () => {
      const { id } = await storage.exemplars.insert("team_local", makeExemplarInput());
      await storage.exemplars.setVerdict("team_local", id, { retired: true, outcome: "fail" }, "human");
      const res = await storage.exemplars.setVerdict("team_local", id, { retired: false }, "human");
      expect(res.status).toBe("applied");
      const fetched = await storage.exemplars.getById("team_local", id);
      expect(fetched!.retiredAt).toBeNull();
    ...
    it("human wins: an llm verdict no-ops on a human-sourced row", async () => {
      const { id } = await storage.exemplars.insert("team_local", makeExemplarInput());
      await storage.exemplars.setVerdict("team_local", id, { retired: true }, "human");
      const res = await storage.exemplars.setVerdict("team_local", id, { retired: false }, "llm");
    ...
    it("an llm verdict applies on an llm-sourced (default) row", async () => {
      const { id } = await storage.exemplars.insert("team_local", makeExemplarInput());
      const res = await storage.exemplars.setVerdict("team_local", id, { retired: true }, "llm");
      expect(res.status).toBe("applied");
      expect((await storage.exemplars.getById("team_local", id))!.labelSource).toBe("llm");
    });

    it("setVerdict on a missing id reports not_found", async () => {
      const res = await storage.exemplars.setVerdict("team_local", "nope", { retired: true }, "human");
    ...
    it("applyBucketCap evicts oldest rows beyond the cap", async () => {
      ...
      await makeN(5);
      const deleted = await storage.exemplars.applyBucketCap("team_local", "install-test", 3);
      expect(deleted).toBe(2);
    });

    it("searchByVector excludes retired exemplars but getById still returns them", async () => {
      const inp = makeExemplarInput();
      const { id } = await storage.exemplars.insert("team_local", inp);
      await storage.exemplars.upsertEmbedding("team_local", id, unitVec(0));
      // present before retire
      const before = await storage.exemplars.searchByVector("team_local", unitVec(0), { installScope: inp.installScope, k: 5 });
      expect(before.map((h) => h.id)).toContain(id);
      // retire → excluded from search, still in getById
      await storage.exemplars.setVerdict("team_local", id, { retired: true }, "human");
      const after = await storage.exemplars.searchByVector("team_local", unitVec(0), { installScope: inp.installScope, k: 5 });
      expect(after.map((h) => h.id)).not.toContain(id);
      expect(await storage.exemplars.getById("team_local", id)).not.toBeNull();
    });
  });
}
