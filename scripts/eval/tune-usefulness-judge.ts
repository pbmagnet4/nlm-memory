...
const DEFS = "used = the response clearly drew on a SPECIFIC fact/value/name/detail from the injected context that is NOT in the prompt and NOT generic. partial = the injected context is on-topic and plausibly informed the response, but there is no specific borrowed detail. unused = the injected context is off-topic or absent from the response. Topical word-overlap ALONE is never 'used'.";

const STYLES = {
  naive: {
    format: { type: "object", properties: { verdict: { type: "string", enum: ["used", "partial", "unused"] } }, required: ["verdict"] },
    system: `Judge whether the assistant RESPONSE used information from the INJECTED context. ${DEFS} Output {"verdict":"..."}.`,
  },
  grounded: {
...
async function judge(g: Gold): Promise<V> {
  ...
  try {
    const r = await fetch(`${URL}/api/chat`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: MODEL, stream: false, think: false, format: cfg.format, options: { temperature: 0 }, messages: [{ role: "system", content: cfg.system }, { role: "user", content: `USER PROMPT:\n${g.prompt}\n\nINJECTED CONTEXT:\n${g.context}\n\nASSISTANT RESPONSE:\n${g.response}` }] }) });
...
const n = gold.length;
console.log(`tune-usefulness-judge — ${MODEL} / style=${STYLE} vs frontier gold (n=${n})`);
